    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
          <TabsList className="bg-slate-800 border border-slate-700 flex flex-wrap">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {["Primaria 1-A", "Primaria 1-B", "Primaria 2-A", "Secundaria 1-A"].map((group) => (
                    <Card key={group} className="border-slate-600 bg-slate-700/30 p-3 sm:p-4">
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <p className="font-medium text-white text-sm sm:text-base truncate">{group}</p>
                          <p className="text-xs sm:text-sm text-slate-400">28 estudiantes</p>
                        </div>
                        <Button
                          variant="outline"
                          size="sm"
                          className="border-slate-600 text-slate-300 bg-transparent text-xs flex-shrink-0"
                        >
                          Editar
                        </Button>
                      </div>
                ...
                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm">Crear Nuevo Grupo</Button>
              </div>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="import" className="space-y-4">
            <Card className="border-slate-700 bg-slate-800/50 p-4 sm:p-6">
              ...
              <div className="space-y-4">
                <div className="border-2 border-dashed border-slate-600 rounded-lg p-6 sm:p-8 text-center">
                  <Upload className="w-10 sm:w-12 h-10 sm:h-12 text-slate-400 mx-auto mb-3" />
                  <p className="text-sm sm:text-base text-slate-300 mb-2">
                    Arrastra tu archivo Excel aquí o haz clic para seleccionar
                  </p>
                  <p className="text-xs text-slate-500 mb-4">Formato: .xlsx, .xls (máx. 5MB)</p>
                  <Button className="bg-blue-600 hover:bg-blue-700 text-white text-sm">Seleccionar Archivo</Button>
                </div>
                <div className="bg-slate-700/30 p-3 sm:p-4 rounded-lg">
                  <p className="text-xs sm:text-sm text-slate-300 mb-2 font-medium">Columnas esperadas:</p>
          ...
          <TabsContent value="settings" className="space-y-4">
            <Card className="border-slate-700 bg-slate-800/50 p-4 sm:p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs sm:text-sm font-medium text-slate-300">Sensibilidad de Alertas</label>
                ...
                <div>
                  <label className="text-xs sm:text-sm font-medium text-slate-300">Intervalo de Análisis</label>
                  <select className="w-full mt-2 bg-slate-700 border border-slate-600 text-white rounded px-3 py-2 text-sm">
          ...
          <TabsContent value="design" className="space-y-4">
            <Card className="border-slate-700 bg-slate-800/50 p-4 sm:p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs sm:text-sm font-medium text-slate-300">Tema</label>
                  <select className="w-full mt-2 bg-slate-700 border border-slate-600 text-white rounded px-3 py-2 text-sm">
                ...
                <div>
                  <label className="text-xs sm:text-sm font-medium text-slate-300">Color Primario</label>
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {["#3B82F6", "#10B981", "#F59E0B", "#EF4444"].map((color) => (
                      <button
                        key={color}
                        className="w-8 h-8 sm:w-10 sm:h-10 rounded border-2 border-slate-600"
                        style={{ backgroundColor: color }}
                      />
                    ))}
                  </div>
                </div>
                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white text-sm">Guardar Diseño</Button>
              </div>
            </Card>
          </TabsContent>
        </Tabs>
      ...
      {/* AI Chatbot */}
      <AIChatbot pageContext="Panel de Administración" />
    </DashboardLayout>
  )
}
